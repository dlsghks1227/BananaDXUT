...
void Stage1Scene::OnEnterScene()
{
	...
	stageComponent->Initialize(player.get(), m_objectCollection.get(), &m_textureAllocator, L"Res/stage1.jpg");
...
void Stage1Scene::OnExitScene()
{
	m_objectCollection.reset();
}

void Stage1Scene::OnUpdate(float fElapsedTime)
{
...
void Stage1Scene::OnLateUpdate(float fElapsedTime)
{
	...
	if (m_player != nullptr && m_stage != nullptr)
	{
		...
		claerMapString << stageComponent->GetClearMap() * 100.0f << L" %";;
...
void Stage1Scene::OnRender(float fElapsedTime)
{
	m_objectCollection->OnRender(fElapsedTime);
}

void Stage1Scene::OnUIRender(float fElapsedTime)
{
	m_HUD.OnRender(fElapsedTime);
	m_UI.OnRender(fElapsedTime);
}

void Stage1Scene::OnResetDevice()
{
}

void Stage1Scene::OnLostDevice()
{
}

void Stage1Scene::OnDestroyDevice()
{
	m_objectCollection.reset();
}

LRESULT Stage1Scene::MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing, void* pUserContext)
{
